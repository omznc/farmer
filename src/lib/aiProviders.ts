...
export async function summarizeWithProvider(
  ...
  console.log(`[AI Provider] Using ${provider.type} for ${commits.length} commits`);
...
async function summarizeWithOpenCode(
  ...
  if (!result) {
    ...
    throw new Error("OpenCode returned empty response. The model may have output to stderr or failed to generate text. Try a different AI provider.");
  }
...
async function summarizeWithOpenAI(
  ...
  if (!response.ok) {
    ...
    throw new Error(`OpenAI API request failed: ${response.statusText} - ${error}`);
  }

  const data = await response.json();
  console.log("[OpenAI] Got response, message length:", data.choices[0].message.content.length);
