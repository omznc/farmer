...
import { RefreshCw, Check, Settings as SettingsIcon } from "lucide-react";
...
export function SettingsView() {
  ...
  const [gitAuthors, setGitAuthors] = useState((workSchedule?.gitAuthors || []).join(", "));
  ...
  const handleDiscoverProviders = async () => {
    ...
    try {
      ...
      const newProviders = discovered.filter(p => !existingIds.includes(p.id));
  ...
  const handleAddCustomProvider = () => {
    ...
    const isValid = customProvider.name && 
      (isOpenAI ? customProvider.config?.apiKey : customProvider.config?.baseUrl);
    ...
    const updatedProviders = aiConfig.providers.map(p => ({ ...p, enabled: false }));
      ...
      <div className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-fg-muted mt-1">
                Attribute weekend commits to the previous Friday or the next Monday
              </p>
            </div>
            ...
            <div>
              ...
              />
              <p className="text-xs text-fg-muted mt-1">
                Enter git author names or emails (comma-separated) to filter commits. Only commits from these authors will be included in time tracking.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-border bg-bg-secondary p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              <RefreshCw className={`w-3 h-3 mr-1.5 ${isDiscovering ? 'animate-spin' : ''}`} />
              Discover
            </Button>
          </div>
          ...
          {aiConfig.providers.length === 0 ? (
            <p className="text-sm text-fg-secondary">
              No AI providers configured. Click "Discover" to auto-detect installed providers.
            </p>
          ) : (
          ...
          <div className="mt-6 pt-6 border-t border-border">
            <h4 className="text-xs font-medium text-fg-primary mb-3">Add Custom Provider</h4>
            <div className="space-y-3">
              <select
                value={customProvider.type}
                onChange={(e) => setCustomProvider({
                  type: e.target.value as AIProvider["type"],
                  name: "",
                  config: e.target.value === "openai" 
                    ? { apiKey: "", model: "" }
                    : { baseUrl: "", model: "" },
                })}
              ...
              >
                ...
                <option value="openapi">OpenAPI Compatible (Ollama, etc.)</option>
              </select>
              <input
                type="text"
              ...
              {customProvider.type === "openai" ? (
                <input
                  ...
                  onChange={(e) => setCustomProvider({
                    ...customProvider,
                    config: { ...customProvider.config, apiKey: e.target.value },
                  })}
                  className="w-full px-3 py-2 rounded-md border border-border bg-bg-tertiary text-sm text-fg-primary focus:outline-none focus:ring-2 focus:ring-accent"
                />
              ) : (
                <input
                  ...
                  onChange={(e) => setCustomProvider({
                    ...customProvider,
                    config: { ...customProvider.config, baseUrl: e.target.value },
                  })}
                  className="w-full px-3 py-2 rounded-md border border-border bg-bg-tertiary text-sm text-fg-primary focus:outline-none focus:ring-2 focus:ring-accent"
                />
              )}
              <input
                type="text"
                placeholder={customProvider.type === "openai" ? "Model (e.g., gpt-4o-mini)" : "Model (e.g., qwen2.5-coder:3b)"}
              ...
              <Button
                ...
                disabled={
                  !customProvider.name || 
                  (customProvider.type === "openai" ? !customProvider.config?.apiKey : !customProvider.config?.baseUrl)
                }
              >
                Add Provider
              </Button>
          ...
          <div className="mt-6 pt-6 border-t border-border">
            <h4 className="text-xs font-medium text-fg-primary mb-3">Custom Prompt Instructions</h4>
            <p className="text-xs text-fg-muted mb-3">
              Add custom instructions to modify the AI summary behavior. This will be appended to the default prompt.
            </p>
            <textarea
