...
import { FolderOpen, Clock, Calendar, RefreshCw } from "lucide-react";
...
export function RepositoryView() {
  ...
  const reposToAnalyze = activeRepos.length > 0 ? activeRepos : (repoPath ? [repoPath] : []);

  useEffect(() => {
    if (reposToAnalyze.length > 0 && workDays.length === 0 && !hasAutoLoaded.current) {
      hasAutoLoaded.current = true;
      console.log("[Repository] Auto-loading repositories on mount");
      handleAnalyzeRepositories(reposToAnalyze);
    }
  }, [reposToAnalyze.length, workDays.length]);

  const handleAnalyzeRepositories = async (paths: string[], overrideFilter?: boolean) => {
    ...
    try {
      ...
      commitsByDate.forEach((commits, date) => {
        ...
        mergedWorkDays.push({
          ...
          firstCommitTime: firstCommit.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true }),
          lastCommitTime: lastCommit.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true }),
        });
      });

      mergedWorkDays.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  return (
    <div className="flex flex-col gap-6 p-8">
        ...
        <div className="rounded-lg border border-border bg-bg-secondary p-4">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <p className="text-xs text-fg-secondary mb-2">Active Repositories ({activeRepos.length})</p>
              <div className="flex flex-wrap gap-2">
                {activeRepos.map((path, index) => {
                  ...
                  return (
                    ...
                    >
                      <span className="max-w-[150px] truncate" title={path}>{name}</span>
                    </div>
                  );
                })}
              </div>
                ...
                >
                  <button
                    onClick={() => {
                      toggleActiveRepo(path);
                      setTimeout(() => {
                        const newActiveRepos = useAppStore.getState().activeRepos;
                        handleAnalyzeRepositories(newActiveRepos.length > 0 ? newActiveRepos : [path]);
                      }, 0);
                    }}
                  ...
                  >
                    {isActive && (
                      <svg className={`w-3 h-3 absolute inset-0.5 ${color.text}`} viewBox="0 0 12 12" fill="none">
                  ...
                  {isActive && (
                    <span className={`text-xs px-2 py-0.5 rounded ${color.bg} ${color.text}`}>
                      {name}
                    </span>
                  )}
                </div>
      ...
      {!isLoading && filteredWorkDays.length === 0 && activeRepos.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 rounded-lg border border-dashed border-border">
          ...
          <p className="text-sm text-fg-muted">Select one or more repositories from the list above</p>
        </div>
      )}

      {!isLoading && filteredWorkDays.length === 0 && activeRepos.length > 0 && workDays.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 rounded-lg border border-dashed border-border">
          <FolderOpen className="w-12 h-12 text-fg-muted mb-4" />
      ...
      {!isLoading && filteredWorkDays.length > 0 && (
        <div className="space-y-4">
          ...
          <CommitTimeline workDays={filteredWorkDays} repoColorMap={repoColorMap} />
        </div>
      )}
    </div>
  );
